...
#ifdef __CLEAR_STREAM__
cudaStream_t streamJr, streamJz, streamJphi;
void createStream()
{
    cudaStreamCreate(&streamJr);
    cudaStreamCreate(&streamJz);
    cudaStreamCreate(&streamJphi);
}
void destroyStream()
{
    cudaStreamDestroy(streamJr);
    cudaStreamDestroy(streamJz);
    cudaStreamDestroy(streamJphi);
}
#endif
...
void NodeField_Cyl3D::clear_current_density_cuda()
{
    ...
    cudaMemset(d_Jz, 0.0, (n_cell_z + 1) * (n_cell_r + 1) * m_phi_number * sizeof(double));
    cudaMemset(d_Jr, 0.0, (n_cell_z + 1) * (n_cell_r + 1) * m_phi_number * sizeof(double));
    cudaMemset(d_Jphi, 0.0, (n_cell_z + 1) * (n_cell_r + 1) * m_phi_number * sizeof(double));
    cudaMemset(d_Rho, 0.0, (n_cell_z + 1) * (n_cell_r + 1) * m_phi_number * sizeof(double));
