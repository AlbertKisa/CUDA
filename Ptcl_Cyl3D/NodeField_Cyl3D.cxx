...
NodeField_Cyl3D::~NodeField_Cyl3D()
{
	...
	if (m_Ezr_Ptr != NULL)
		aligned_free(m_Ezr_Ptr);
	if (m_Ephi_Ptr != NULL)
		aligned_free(m_Ephi_Ptr);
	if (m_Mzr_Ptr != NULL)
		aligned_free(m_Mzr_Ptr);
	if (m_Mphi_Ptr != NULL)
		aligned_free(m_Mphi_Ptr);
	edges_z_current.clear();
	Jz_to_edges_z.clear();
	edges_r_current.clear();
	Jr_to_edges_r.clear();
...
void NodeField_Cyl3D::fill_with_B(const IndexAndWeights_Cyl3D &iw, TxVector<double> &b_field)
{
	...
	b_temp[0] += Bz_static.GetValue(i, j) * (wl[0] * wl[1]) + Bz_static.GetValue(i + 1, j) * (wu[0] * wl[1]) + Bz_static.GetValue(i, j + 1) * (wl[0] * wu[1]) + Bz_static.GetValue(i + 1, j + 1) * (wu[0] * wu[1]);

	b_temp[1] += Br_static.GetValue(i, j) * (wl[0] * wl[1]) + Br_static.GetValue(i + 1, j) * (wu[0] * wl[1]) + Br_static.GetValue(i, j + 1) * (wl[0] * wu[1]) + Br_static.GetValue(i + 1, j + 1) * (wu[0] * wu[1]);
	b_field = b_temp;
}
...
void NodeField_Cyl3D::fill_with_Rho(TxVector<double> &pos, double &Rho_field)
{
	...
	if (j == 1)
	{
		...
		Rho_tmp += Rho_ijk * (wl[0] * wl[1] * wl[2]) + Rho_i1jk * (wu[0] * wl[1] * wl[2]) + Rho_ij1k * (wl[0] * wu[1] * wl[2]) + Rho_i1j1k * (wu[0] * wu[1] * wl[2]);

		Rho_tmp += Rho_ijk1 * (wl[0] * wl[1] * wu[2]) + Rho_i1jk1 * (wu[0] * wl[1] * wu[2]) + Rho_ij1k1 * (wl[0] * wu[1] * wu[2]) + Rho_i1j1k1 * (wu[0] * wu[1] * wu[2]);
		Rho_field = Rho_tmp;
	}
	else
	{
...
void NodeField_Cyl3D::fill_with_J(const IndexAndWeights_Cyl3D &iw, TxVector<double> &density)
{
	...
	if (j == 1)
	{
		...
		density_tmp += Jphi_ijk * (wl[0] * wl[1]) + Jphi_i1jk * (wu[0] * wl[1]) + Jphi_ij1k * (wl[0] * wu[1]) + Jphi_i1j1k * (wu[0] * wu[1]);
