...
void ZRGrid_Ctrl::CheckPorts()
{
  ...
  for(Iter.Initialize(thePorts); Iter.More(); Iter.Next() ){
    ...
    if( !IsPMLPortType(thePortType) ) continue;

    GetModelsCtrl()->ComputePortDirWithFaceIndexOfPort(thePortIndex, theBaryCenter, theLineDir, theRelativeDir);

    if(theLineDir==DIRX){
      if(theRelativeDir==1){
	if(!m_UpperBndsIsSetAsPort[0]) m_UpperBndsIsSetAsPort[0]=true;
      }else{
	if(!m_LowerBndsIsSetAsPort[0]) m_LowerBndsIsSetAsPort[0]=true;
      }
    }else if(theLineDir==DIRY){
      if(theRelativeDir==1){
	if(!m_UpperBndsIsSetAsPort[1]) m_UpperBndsIsSetAsPort[1]=true;
      }else{
	if(!m_LowerBndsIsSetAsPort[1]) m_LowerBndsIsSetAsPort[1]=true;
      }
    }else if(theLineDir==DIRZ) {
      if(theRelativeDir==1){
	if(!m_UpperBndsIsSetAsPort[2]) m_UpperBndsIsSetAsPort[2]=true;
      }else{
	if(!m_LowerBndsIsSetAsPort[2]) m_LowerBndsIsSetAsPort[2]=true;
      }
    }else{
    ...
    if(IsInputPMLPortType(thePortType)){
      if(theLineDir==DIRX){
	if(theRelativeDir==1){
	  if(!m_UpperBndsIsSetAsInputPort[0]) m_UpperBndsIsSetAsInputPort[0]=true;
	}else{
	  if(!m_LowerBndsIsSetAsInputPort[0]) m_LowerBndsIsSetAsInputPort[0]=true;
	}
      }else if(theLineDir==DIRY){
	if(theRelativeDir==1){
	  if(!m_UpperBndsIsSetAsInputPort[1]) m_UpperBndsIsSetAsInputPort[1]=true;
	}else{
	  if(!m_LowerBndsIsSetAsInputPort[1]) m_LowerBndsIsSetAsInputPort[1]=true;
	}
      }else if(theLineDir==DIRZ) {
	if(theRelativeDir==1){
	  if(!m_UpperBndsIsSetAsInputPort[2]) m_UpperBndsIsSetAsInputPort[2]=true;
	}else{
	  if(!m_LowerBndsIsSetAsInputPort[2]) m_LowerBndsIsSetAsInputPort[2]=true;
	}
      }else{
