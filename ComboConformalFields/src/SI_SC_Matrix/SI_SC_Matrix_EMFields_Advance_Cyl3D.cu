...
void SI_SC_Matrix_EMFields_Cyl3D::cpuOp1()
{
    m_FldSrcs_Cyl3D->Advance();
    m_FldSrcs_Cyl3D->Advance_SI_MJ(1.0);
    m_FldSrcs_Cyl3D->Advance_SI_Mag_Damping_0(1.0);
}

void SI_SC_Matrix_EMFields_Cyl3D::setGraph1()
{
    ErrorCheck(cudaStreamCreate(&stream), __FILE__, __LINE__);
    cudaGraph_t graph;
    ErrorCheck(cudaStreamBeginCapture(stream, cudaStreamCaptureModeGlobal), __FILE__, __LINE__);

    dim3 block(256);
    dim3 grid((unsigned int)ceil(m_MphiDatasNum / (float)block.x));

    g_AdvanceMagCntr_WithDamping_m_Mphi<<<grid, block, 0, stream>>>(m_h_d_MphiFuncArray_Cyl3D, m_MphiDatasNum,
                                                                    m_h_d_MphiDatasPtr, m_h_d_EzrDatasPtr);

    block.x = 256;
    grid.x = ((unsigned int)ceil(m_MzrDatasNum / (float)block.x));

    g_AdvanceMagCntr_WithDamping_m_Mzr<<<grid, block, 0, stream>>>(m_h_d_MzrFuncArray_Cyl3D, m_MzrDatasNum,
                                                                   m_h_d_MzrDatasPtr, m_h_d_EphiDatasPtr, m_h_d_EzrDatasPtr);

    ErrorCheck(cudaStreamEndCapture(stream, &graph), __FILE__, __LINE__);
    ErrorCheck(cudaGraphInstantiate(&graphExec1, graph, NULL, NULL, 0), __FILE__, __LINE__);
    ErrorCheck(cudaGraphDestroy(graph), __FILE__, __LINE__);
    std::cout << "setGraph1 end" << std::endl;
}

void SI_SC_Matrix_EMFields_Cyl3D::cpuOp2()
{
    m_FldSrcs_Cyl3D->Advance_SI_Mag_Damping_1(1.0);
    m_FldSrcs_Cyl3D->Advance_SI_J(1.0);
    m_FldSrcs_Cyl3D->Advance_SI_Elec_Damping_0(1.0, m_Damping);
}

void SI_SC_Matrix_EMFields_Cyl3D::setGraph2()
{
    cudaGraph_t graph;
    ...
    g_AdvanceElecCntr_WithDamping_m_Ezr<<<grid, block, 0, stream>>>(m_h_d_damping_scale, m_h_d_EzrFuncArray_Cyl3D, m_EzrDatasNum,
                                                                    m_h_d_EzrDatasPtr, m_h_d_MphiDatasPtr, m_h_d_MzrDatasPtr);

    block.x = 256;
    grid.x = (unsigned int)ceil(m_EphiDatasNum / (float)block.x);

    g_AdvanceElecCntr_WithDamping_m_Ephi<<<grid, block, 0, stream>>>(m_h_d_damping_scale, m_h_d_EphiFuncArray_Cyl3D, m_EphiDatasNum,
                                                                     m_h_d_EphiDatasPtr, m_h_d_MzrDatasPtr);

    block.x = 256;
    grid.x = (unsigned int)ceil(m_EAxisDatasNum / (float)block.x);

    g_AdvanceElecCntr_WithDamping_m_EAxis<<<grid, block, 0, stream>>>(m_h_d_Parameters, m_h_d_EAxisFuncArray_Cyl3D, m_EAxisDatasNum,
    ...
    g_AdvanceElecCntr_WithDamping_m_EAxis_1<<<grid, block, 0, stream>>>(m_h_d_Parameters, m_h_d_EAxisDualContourValue, m_h_d_EAxisPhysDataJ, m_h_d_EAxisPhysDataE, m_EAxisDatasNum / phi_num);

    block.x = 256;
    grid.x = (unsigned int)ceil(m_EAxisDatasNum / (float)block.x);

    g_AdvanceElecCntr_WithDamping_m_EAxis_2<<<grid, block, 0, stream>>>(m_h_d_Parameters, m_h_d_EAxisFuncArray_Cyl3D, m_EAxisDatasNum,
                                                                        m_h_d_EAxisDualContourValue, m_h_d_EAxisPhysDataJ, m_h_d_EAxisPhysDataE);

    block.x = 256;
    grid.x = (unsigned int)ceil(m_EAxisDatasNum / phi_num / (float)block.x);

    g_AdvanceElecCntr_WithDamping_m_EAxis_3<<<grid, block, 0, stream>>>(m_h_d_EAxisDualContourValue, m_h_d_EAxisPhysDataJ, m_h_d_EAxisPhysDataE, m_EAxisDatasNum / phi_num);

    block.x = 256;
    grid.x = (unsigned int)ceil(m_EAxisDatasNum / (float)block.x);

    g_AdvanceElecCntr_WithDamping_m_EAxis_end<<<grid, block, 0, stream>>>(m_h_d_damping_scale, m_h_d_EAxisFuncArray_Cyl3D, m_EAxisDatasNum,
                                                                          m_h_d_EzrDatasPtr);

    block.x = (256);
    grid.x = ((unsigned int)ceil(m_Mur_EzrDatasNum / (float)block.x));

    g_AdvanceElecMur_WithDamping_m_Ezr<<<grid, block, 0, stream>>>(m_h_d_damping_scale, m_h_d_MurEzrFuncArray_Cyl3D, m_Mur_EzrDatasNum,
                                                                   m_h_d_EzrDatasPtr);

    block.x = 256;
    grid.x = (unsigned int)ceil(m_Mur_EphiDatasNum / (float)block.x);

    g_AdvanceElecMur_WithDamping_m_Ephi<<<grid, block, 0, stream>>>(m_h_d_damping_scale, m_h_d_MurEphiFuncArray_Cyl3D, m_Mur_EphiDatasNum,
                                                                    m_h_d_EphiDatasPtr);

    cudaStreamEndCapture(stream, &graph);
    cudaGraphInstantiate(&graphExec2, graph, NULL, NULL, 0);
    cudaGraphDestroy(graph);
}

void SI_SC_Matrix_EMFields_Cyl3D::cpuOp3()
{
...
void SI_SC_Matrix_EMFields_Cyl3D::setGraph3()
{
    ...
    g_AdvanceElecMurVoltage_WithDamping_m_Ezr<<<grid, block, 0, stream>>>(m_h_d_damping_scale, m_h_d_MurVoltageEzrFuncArray_Cyl3D, m_MurVoltage_EzrDatasNum,
                                                                          m_h_d_EzrDatasPtr, h_d_Ebar, h_d_Ebar2);

    block.x = 256;
    grid.x = (unsigned int)ceil(m_MurVoltage_EphiDatasNum / (float)block.x);

    g_AdvanceElecMurVoltage_WithDamping_m_Ephi<<<grid, block, 0, stream>>>(m_h_d_damping_scale, m_h_d_MurVoltageEphiFuncArray_Cyl3D, m_MurVoltage_EphiDatasNum,
                                                                           m_h_d_EphiDatasPtr);

    cudaStreamEndCapture(stream, &graph);
    cudaGraphInstantiate(&graphExec3, graph, NULL, NULL, 0);
    cudaGraphDestroy(graph);
}

void SI_SC_Matrix_EMFields_Cyl3D::cpuOp4()
{
...
void SI_SC_Matrix_EMFields_Cyl3D::EMForLoop()
{
    cpuOp1();
...
void SI_SC_Matrix_EMFields_Cyl3D::clear()
{
...
__global__ void g_AdvanceMagCntr_MphiMzr(FIT_Mag_Func_Cyl3D *MphiFuncArray, FIT_Mag_Func_Cyl3D *MzrFuncArray,
                                         Standard_Size MphiDatasNum, Standard_Size MzrDatasNum,
                                         Standard_Real *MphiDatasPtr, Standard_Real *EzrDatasPtr, Standard_Real *MzrDatasPtr, Standard_Real *EphiDatasPtr)
{
    int threadID = blockDim.x * blockIdx.x + threadIdx.x;
    if (threadID < MphiDatasNum)
    {
...
__global__ void g_AdvanceElecCntr_MphiMzr(Standard_Real *theta, FIT_Elec_Func_Cyl3D *EphiFuncArray, FIT_Elec_Func_Cyl3D *EzrFuncArray,
                                          Standard_Size EphiDatasNum, Standard_Size EzrDatasNum,
                                          Standard_Real *EphiDatasPtr, Standard_Real *MzrDatasPtr, Standard_Real *EzrDatasPtr, Standard_Real *MphiDatasPtr)
{
    int threadID = blockDim.x * blockIdx.x + threadIdx.x;
    if (threadID < EzrDatasNum)
    {
    ...
    AdvanceMagCntr_WithDamping()
{
#ifdef __CUDA__
    // @HappyzzZ
    // printf("m_MphiDatasNum: %d\n", m_MphiDatasNum);
    // printf("m_MzrDatasNum: %d\n", m_MzrDatasNum);
    ...
    // // 修改后
    // dim3 block(256);
    // Cempic_Size maxSize = m_MphiDatasNum > m_MzrDatasNum ? m_MphiDatasNum : m_MzrDatasNum;
    // dim3 grid((unsigned int)ceil(maxSize / (float)block.x));

    // g_AdvanceMagCntr_MphiMzr<<<grid, block>>>(m_h_d_MphiFuncArray_Cyl3D, m_h_d_MzrFuncArray_Cyl3D, m_MphiDatasNum, m_MzrDatasNum,
    //                                           m_h_d_MphiDatasPtr, m_h_d_EzrDatasPtr, m_h_d_MzrDatasPtr, m_h_d_EphiDatasPtr);
...
{
#ifdef __CUDA__
    // printf("m_Mur_EzrDatasNum: %d\n", m_Mur_EzrDatasNum);
    // printf("m_Mur_EphiDatasNum: %d\n", m_Mur_EphiDatasNum);
    ...
    AdvanceElecMurVoltage_WithDamping(const Standard_Real damping_scale)
{
    ...
    // printf("m_MurVoltage_EzrDatasNum: %d\n", m_MurVoltage_EzrDatasNum);
    // printf("m_MurVoltage_EphiDatasNum: %d\n", m_MurVoltage_EphiDatasNum);
