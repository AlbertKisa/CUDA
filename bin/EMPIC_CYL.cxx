...
int main(int argc, char **argv)
{
	...
	if (dimPhi >= 2)
	{
		...
		for (int i = 0; i <= nstep; i++)
		{
			...
			// // @HappyzzZ
			// theEMFields_Cyl3D->EMForLoop();
			// // @HappyzzZ

			theEMFields_Cyl3D->AdvanceWithDamping(); // include paticle source
			// EMFields_Cyl3D->AdvanceWithDamping();
...
#ifdef __CUDA__
			// @HappyzzZ
			// time_elapsed_ = node_field_Cyl3D.step_to_conformal_current_test_cuda();
